...
docker run --rm --name $docker_name -e block_time=1 -p $port:$port $flextesa_image flextesa key bob
docker run --rm --name $docker_name -e block_time=1 -p $port:$port $flextesa_image delphibox start
